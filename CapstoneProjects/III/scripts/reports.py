...
def generate_total_sales_bar_chart(table_data):
  ...
  data = [ table_data['total_revenue'].tolist()[:10]]
  ...
  bc.valueAxis.valueMin = table_data['total_revenue'][:10].min()  - 20000
  bc.valueAxis.valueMax = table_data['total_revenue'][:10].max()  + 10000
  ...
  bc.categoryAxis.categoryNames = table_data['car.car_model'].tolist()[:10]

  drawing.add(bc)
  return drawing

def generate(filename, title, additional_info, table_data, max_sales_by_car,top_ten):
